...
import { concatInt16Arrays, calculateVolumeWithFloat32Array } from './AudioUtils';
...
export default class MicrophoneAudioSource extends AudioSource {
  ...
  constructor(options?: MicrophoneAudioSourceOptions) {
    ...
    const callback = (stream: any) => {
      const resampler: any = new global.Resampler(this._audioContext.sampleRate, this._targetSampleRate, 1, 2048);
      ...
      this._processor.onaudioprocess = (event) => {
        ...
        this.sendAudio(wav.data.samples, calculateVolumeWithFloat32Array(inputData));
        if (this._captureAudio) {
          this._audioData = concatInt16Arrays(this._audioData, int16Samples); //new Int16Array(wav.data.samples));
        }
    ...
    navigator.getUserMedia.call(navigator, { audio: true }, callback, () => { });
  }
